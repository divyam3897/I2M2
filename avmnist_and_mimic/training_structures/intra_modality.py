...
from .inter_modality import MMDL, deal_with_objective
#import pdb
...
def train(
        unimodal_models, train_dataloader, valid_dataloader, total_epochs, save_dir, additional_optimizing_modules=[], is_packed=False,
    ...
    """
    total_parameters = list(unimodal_models[0].parameters()) + list(unimodal_models[1].parameters())
                ...
                if is_packed:
                    with torch.backends.cudnn.flags(enabled=False):
                        ...
                        for idx, uni_model in enumerate(unimodal_models):
                            if idx == 0:
                                out = (uni_model(j[idx].float().to(device)))
                            else:
                                out += (uni_model(j[idx].float().to(device)))
            ...
            if task == "classification":
                ...
                if acc > bestacc:
                    ...
                    for i in range(len(unimodal_models)):
                        torch.save(unimodal_models[i], f'{save_dir}/ens_unimodal_{i}.pt')
                else:
                    patience += 1
            elif task == "multilabel":
...
def single_test(
        unimodal_models, test_dataloader, is_packed=False,
    ...
    with torch.no_grad():
        ...
        for j in test_dataloader:
            if is_packed:
                ...
                output_1 = unimodal_models[0](j[0].float().to(device))
                output_2 = unimodal_models[1](j[1].float().to(device))
                out = torch.log((torch.softmax(output_1, dim=-1) + torch.softmax(output_2, dim=-1))/2.)
...
def test(
        unimodal_models, test_dataloaders_all, dataset='default', method_name='My method', is_packed=False, criterion=nn.CrossEntropyLoss(), task="classification", auprc=False, input_to_float=True, no_robust=False):
    """
    ...
    if no_robust:
        def _testprocess():
            single_test(unimodal_models, test_dataloaders_all, is_packed,
                        criterion, task, auprc, input_to_float)
        all_in_one_test(_testprocess, [unimodal_models])
        return

    def _testprocess():
        single_test(unimodal_models, test_dataloaders_all[list(test_dataloaders_all.keys())[
