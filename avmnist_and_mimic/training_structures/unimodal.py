...
def train(encoders, heads, train_dataloader, valid_dataloader, total_epochs, early_stop=False, optimtype=torch.optim.RMSprop, lr=0.001, weight_decay=0.0, criterion=nn.CrossEntropyLoss(), auprc=False, save_dir='./ckpts/', modalnum=0, task='classification', track_complexity=False):
    ...
    """
    def _trainprocess():
        model = nn.Sequential(encoders[modalnum], heads[modalnum])
        ...
        for epoch in range(total_epochs):
            ...
            if task == "classification":
                ...
                if not auprc and (acc > bestacc):
                    ...
                    bestacc = acc
                    print("Saving Best")
                    torch.save(model, f'{save_dir}/unimodal_{modalnum}.pt')
                ...
                if valloss < bestvalloss:
                    ...
                    bestvalloss = valloss
                    print("Saving Best")
                    torch.save(model, f'uni_{modalnum}.pt')
                else:
                    patience += 1
...
def single_test(model, test_dataloader, auprc=False, modalnum=0, task='classification', criterion=None):
...
def test(model, test_dataloaders_all, dataset='default', method_name='My method', auprc=False, modalnum=0, task='classification', criterion=None, no_robust=False):
